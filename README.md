...
- Streams are read two ways: an inline `<script>` on first paint, and an `XMLHttpRequest` hook on `/ajax/get_cdn_series/` for anything loaded later. Buttons stay disabled until one of them lands.
